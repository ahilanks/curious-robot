...
**Next (user's call, kill pending): `wr_msegate6` = same stack, d-start 6, `--goal-mse-pctl-max 0.3`** — stop the inner walk at the masterable band (0.2 = known-good both runs; 0.3 probes the boundary; 0.2 = conservative fallback). d-mastery then climbs on mostly-predictable goals: the historical capped ladder (d→21) had NO MSE gating, so a capped-band ladder is closer to the proven regime while keeping surprise-seeking at the band top. msegate5 meanwhile rides — its locality-metered climb is independently informative (currently the best amplitude arc of the msegate line).
