...
**LONGER RUN (12k steps) CONFIRMS the radius grows ~3x, then soft-plateaus.** `cem_v7_curriculum_long` (same recipe, `--total-steps 12000 --goal-curric-max-k 30`): the reachable radius climbed **k=6 → 18** (~3x the early ~k5-6 closing limit) — timeline k=6@1050, 10@6000, 13@7800, 16@9000, 17@9600, **18@10050** — while LATENT reach held steady ~**0.09–0.10** across every well-sampled stage (k=6 n=47:0.094; k=9 n=35:0.091; k=12 n=23:0.096; k=17:0.102; k=18:0.089). It then **soft-plateaued at k=18** (last ~1700 steps no further bump; reach ~0.089 sat just below the 0.12 advance threshold) — a SOFT ceiling within a 12k-step run, not a hard wall (reach stayed ~0.09, just shy of the bump trigger). **CONFIRMED: pursuing reachable goals lets consolidation on the directed-reachable trajectories extend the planner's reachable radius SEVERALFOLD while holding reach ~constant — the directed-data bootstrap works.** Net session result: CEM latent goal-reaching went from reach=0 everywhere → reach ~0.1 with a reachable radius that self-extends k6→18 under the curriculum. To push past ~k18: lower the advance threshold, longer run, or tighten the per-step closing precision (finer final approach) — all within the latent-reach objective; qpos remains diagnostic-only.
