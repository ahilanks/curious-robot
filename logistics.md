...
**CORRECTION (user caught it: "in the best run we've ever had we chose the lowest mse states… then slowly ramped up to the hardest"):** the previous paragraph's "historical capped ladder had NO MSE gating" was WRONG. The historical best runs used EXACTLY the full low→high ramp: `--goal-mse-curric` (06-29) walked pctl 0→1.0 in `cem_msecur_h1` (d 12→18 ceiling break, THE lever of that era, default-on since 07-02), and the hot-line d→21 run cycled full pctl ladders per d. Why the same ramp works then / dies at 0.40 now — three regime differences: (1) **the bar**: msecur-era gate = reach ≥ 0.05 windowed (hard-band reach 0.07–0.20 SCRAPED past; never "mastered" by today's standard) vs msegate's arrival ≥ 0.95 — tonight's pctl-0.40 reach ≈ 0.00–0.01 would have frozen the old ladder too, it was just never asked; (2) **frozen vs live encoder**: "encoder frozen ⇒ stable reachability metric" then; consolidate-every-70 + amplitude growth now = the fog moves under pursuit; (3) **buffer wildness**: capped-era high-MSE = odd poses; uncapped-curriculum high-MSE = amplitude-frontier chaos states. Decision reframed for the user: **A** = pctl-max 0.3 (keeps the 0.95-arrival mastery bar, escalate along d) vs **B** = full 0→1.0 ramp with the historical loose reach-gate (replicates the proven regime; rungs mean "visited," not "mastered"). Recommendation stays A (the strict bar is the user's deliberate design); B armed as the faithful-reproduction alternative.
