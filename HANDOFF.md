...
- **git push**: `gh` reads `GH_TOKEN` from the environment (`set -a; source .env; set +a`). The token that sat in the public history was auto-revoked by GitHub secret scanning (09-15). **History purge (09-15): `main` rewritten with `git filter-repo --invert-paths --path .env` and force-pushed (`c769033` → `779cdc2`); backup `/workspace/curious-robot-pre-purge.bundle` on the 09-15 pod. Eight feature branches on origin still carry `.env` in their history (ledger 09-15 later entry lists them + the commands) — rewrite or delete them; every other clone of `main` must re-clone.** Rotate the W&B + Pushover keys that were in the old `.env` regardless. Never print `.env`.
