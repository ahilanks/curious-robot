...
- **git push**: `gh` reads `GH_TOKEN` from the environment (`set -a; source .env; set +a`). The token that sat in the public history was auto-revoked by GitHub secret scanning (09-15); a history purge (`git filter-repo`, installed) is pending the user's go — it rewrites `main` and every other clone must re-clone. Never print `.env`.
