...
README control law:
    ...
    dq_t     = a_t (.) dq_max                 # per-joint *delta* target, dq_max ~= inf
    tau_t    = clip(Kp * dq_t - Kd * qdot_t, -tau_max, tau_max)

The SO101 XML drives each joint with a MuJoCo <position> actuator
(kp=998.22, kv=2.731, forcerange = +/-2.94 / +/-3.35 N.m). A position actuator
applies exactly

    tau = clip(kp * (ctrl - q) - kv * qdot, -forcerange, forcerange).

So if we command a *delta target* ctrl = q + dq, MuJoCo realises the README law
verbatim, with Kp=kp=998.22, Kd=kv=2.731, tau_max=forcerange. No torque actuator
or hand-rolled PD loop is needed; this adapter only converts (action, q) -> ctrl.

Verified servo constants: STS3215 stall torque 30 kg.cm = 2.94 N.m; kp/kv are the
TheRobotStudio SO-ARM101 MuJoCo values (RBE501 derivation at servo P-gain 16).
"""
...
class SOArmAdapter:
    def __init__(
        ...
        action_max: float = 0.3,      # README: action_max (delta scale per step)
        dq_max: float = 100.0,        # README: dq_max ~= inf (clip never binds in practice)
    ):
    ...
    def ctrl_target(self, action: np.ndarray, qpos: np.ndarray) -> np.ndarray:
        ...
        target = clip(q + clip(a * action_max, +/-dq_max), joint_range)
        """
        a = np.clip(np.asarray(action, dtype=np.float64), -1.0, 1.0)
        dq = np.clip(a * self.action_max, -self.dq_max, self.dq_max)
        return np.clip(qpos + dq, self.joint_low, self.joint_high)
