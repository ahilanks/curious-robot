...
from train_decoder import (ckpt_dir, dataset_path, encode_frames, find_weights,    # noqa: E402
                           latent_stats, load_lewm, preprocess, recalibrate_bn, resize_u8)
...
def run_lambda(run: str) -> float:
    from omegaconf import OmegaConf
    cfg = OmegaConf.load(ckpt_dir(run) / "config.yaml")
    return float(cfg.loss.sigreg.weight)
...
@torch.no_grad()
def rollout(model, ctx_u8, actions_norm, T, device, H=3, img_size=224):
    ...
    z = encode_frames(model, ctx_u8, device, img_size=img_size).to(device).unsqueeze(0)     # (1, H, D)
...
def main(a):
    ...
    for run in a.runs:
        ...
        with torch.no_grad():
            z_recon = encode_frames(model, recon_u8, device).to(device)
            rec = dec.to_uint8_hwc(z_recon)
            # latents of the window frames: (N, H+1, D)
            flat = win_frames.reshape(-1, TILE, TILE, 3)
            zw = encode_frames(model, flat, device).to(device).reshape(len(wins), H + 1, -1)
            ...
            z_roll = rollout(model, roll_frames[:H], roll_actions, T, device, H)
            dec_roll = dec.to_uint8_hwc(torch.cat([encode_frames(model, roll_frames[:H], device).to(device), z_roll]))
        m = dict(run=run, lam=lam, epoch=epoch, decoder_val_mse=float(dmeta.get("val_mse", float("nan"))),
        ...
        metrics.append(m)
        rows_recon.append(label_row(list(rec), f"lambda {lam:g}\nval mse {m['decoder_val_mse']:.4f}\neff rank {m['eff_rank']:.0f}/{m['dim']}\nfrac_rand {m['frac_rand']:.2f}"))
        rows_roll.append(label_row(list(dec_roll), f"lambda {lam:g}\ndecode(z ctx | imagined)\npred/persist {m['pred_over_persist']:.2f}"))
        print(f"[compare] {run:14s} lam {lam:<5g} ep {epoch}: dec val mse {m['decoder_val_mse']:.4f}  z_std {m['z_std']:.3f}  "
    ...
    lams = np.array([m["lam"] for m in metrics])
    x = np.where(lams > 0, lams, np.nan)
    x0 = (np.nanmin(x) / 10) if np.isfinite(x).any() else 1e-3
    ...
    keys = [("decoder_val_mse", "decoder val MSE (pixels, [0,1])"), ("eff_rank", "latent effective rank"),
            ...
            ("z_std", "latent per-dim std"), ("sigreg_stat", "SIGReg statistic (per-frame latents)")]
    fig, axes = plt.subplots(2, 3, figsize=(13, 7))
    for ax, (k, title) in zip(axes.ravel(), keys):
        ax.plot(xs, [m[k] for m in metrics], "o-", color="#1f77b4")
        for xi, m in zip(xs, metrics):
            ax.annotate(f"{m['lam']:g}", (xi, m[k]), textcoords="offset points", xytext=(4, 4), fontsize=8)
        ax.set_xscale("log"); ax.set_title(title, fontsize=10); ax.set_xlabel("SIGReg lambda (0 plotted at left)")
        ax.grid(alpha=0.3)
    fig.suptitle(f"LeWM Cube, SIGReg-lambda sweep, epoch {metrics[0]['epoch']}", fontsize=11)
